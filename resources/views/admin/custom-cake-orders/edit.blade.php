...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4">
        <h1 class="h2 text-gray-800 fw-bold">
            <i class="bi bi-pencil-square me-2"></i>Edit Order #{{ $customCakeOrder->id }}
        </h1>
    ...
    <div class="card border-0 shadow-sm rounded-lg glass-card">
        <div class="card-body">
            ...
            <form action="{{ route('custom-cake-orders.update', $customCakeOrder) }}" method="POST" enctype="multipart/form-data" id="editOrderForm">
                @csrf
                @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="user_id" 
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('user_id', $customCakeOrder->user_id) == $user->id ? 'selected' : '' }}>
                                ...
                                <select name="cake_size" 
                                    ...
                                    @foreach($cakeSizes as $size)
                                        <option value="{{ $size->name }}" 
                                                ...
                                                {{ old('cake_size', $customCakeOrder->cake_size) == $size->name ? 'selected' : '' }}>
                                ...
                                <select name="cake_flavor" 
                                    ...
                                    @foreach($cakeFlavors as $flavor)
                                        <option value="{{ $flavor->name }}" 
                                                ...
                                                {{ old('cake_flavor', $customCakeOrder->cake_flavor) == $flavor->name ? 'selected' : '' }}>
                                ...
                                <select name="cake_filling" 
                                    ...
                                    @foreach($cakeFillings as $filling)
                                        <option value="{{ $filling->name }}" 
                                                ...
                                                {{ old('cake_filling', $customCakeOrder->cake_filling) == $filling->name ? 'selected' : '' }}>
                                ...
                                <select name="cake_frosting" 
                                    ...
                                    @foreach($cakeFrostings as $frosting)
                                        <option value="{{ $frosting->name }}" 
                                                ...
                                                {{ old('cake_frosting', $customCakeOrder->cake_frosting) == $frosting->name ? 'selected' : '' }}>
                        ...
                        <div class="col-12">
                            <div class="mb-3">
                                ...
                                <textarea name="special_instructions" 
                                          ...
                                          rows="3">{{ old('special_instructions', $customCakeOrder->special_instructions) }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                @if($customCakeOrder->reference_image)
                                    <div class="mb-2">
                                        ...
                                        <p class="form-text">Current reference image</p>
                                    </div>
                                @endif
                                <input type="file" 
                                ...
                                @error('reference_image')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text">Upload a new reference image for the cake design (optional).</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('price', $customCakeOrder->price) }}" 
                                       required
                                       id="price_input"
                                       step="0.01"
                                       min="0">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="status" 
                                        ...
                                        required>
                                    <option value="pending" {{ old('status', $customCakeOrder->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                    <option value="confirmed" {{ old('status', $customCakeOrder->status) == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                                    <option value="in_progress" {{ old('status', $customCakeOrder->status) == 'in_progress' ? 'selected' : '' }}>In Progress</option>
                                    <option value="completed" {{ old('status', $customCakeOrder->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                                    <option value="cancelled" {{ old('status', $customCakeOrder->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="date" 
                                       ...
                                       value="{{ old('delivery_date', $customCakeOrder->delivery_date ? $customCakeOrder->delivery_date->format('Y-m-d') : '') }}" 
                                       required
                        ...
                        <div class="col-12">
                            <div class="mb-3">
                                ...
                                <textarea name="delivery_address" 
                                          ...
                                          rows="3" 
                                          required>{{ old('delivery_address', $customCakeOrder->delivery_address) }}</textarea>
